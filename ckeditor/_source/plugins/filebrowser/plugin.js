...
( function()
{
	...
	function browseServer( evt )
	{
		...
		var div = $("<div id='ck_"+param.CKEditorFuncNum+"' style='display:none;'></div>");
		div.html('<iframe name="kcfinder_textbox" src="'+ url +'"' + 
				...
				$(".ui-xlgwr>span").click(function () {
					...
					if (spantext == "extlink") {
						...
						if(status && status == 'max'){
							 $dialog.dialog({width: 860, height: 550});
							 $(con).data('status','min');
						}else{
							var w = screen.availWidth - 24;
							var h = $(window).height() - $(".ui-dialog-titlebar",con).height() + 4
							$dialog.dialog({position: ['left', 'top'],width: w, height: h});
							$(con).data('status','max');
						}
	...
	function setUrl( fileUrl, data )
	{
        ...
        var editor=x.getParentEditor();
		var user_path = editor.config["user_files_path"];
		if(user_path) fileUrl = user_path + fileUrl;
		console.log(fileUrl);
		if ( targetInput )
        ...
        $("#ck_"+editor._.filebrowserFn).dialog("destory").remove();		
	}
