    ...
    static function content($dir, array $options=null) {
        ...
        while (($file = @readdir($dh)) !== false) {	
            ...
            if ($options['followLinks'] && ($type === "link")) {
 			...
 			$extension = preg_match('/^.*\.([^\.]*)$/s', $file, $patt) ? strtolower($patt[1]) : "";
			
            if ((($type === "dir") && (($file == ".") || ($file == ".."))) ||
                !preg_match($options['pattern'], $file) 
				||((is_array($options['followExts'])) && !in_array($extension, $options['followExts']))
            )
                continue;
		...
		//$files[] = $options['addPath'] ? "$dir/$filename" : $filename;
        }
        closedir($dh);
