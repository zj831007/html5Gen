...
require_once $_SERVER['DOCUMENT_ROOT'] . "/member/member_check.inc.php";
class uploader {
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct() {
		...
		global $user_profile;
		if(isset($user_profile) && isset($user_profile['user_files_path'])){
		    $this->config['uploadURL'] = "../../".$user_profile['user_files_path']."resource/";
		}else{
		    $this->label("user profile error");
		}
        ...
        if (preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/(.+)\/?$/',
			...
			$this->theDir = "{$this->config['uploadDir']}";
            $this->theURL = "{$this->config['uploadURL']}";
       ...
       /* if (isset($this->config['_check4htaccess']) &&
        ...
        if (!is_dir($this->theDir)) {
            if (!mkdir($this->theDir, $this->config['dirPerms']))
                $this->backMsg("Cannot create {dir} folder.", array('dir' => $this->theDir));
        } elseif (!is_readable($this->theDir))
    ...
    protected function makeThumb($file, $overwrite=true) {
		...
		if($gd->type == IMAGETYPE_PNG)
		    return $this->pngthumb($file, $thumb, $this->config['thumbWidth'],$this->config['thumbHeight']);
	    else
            return $gd->imagejpeg($thumb, $this->config['jpegQuality']);
    }
	
	protected function pngthumb($sourePic,$smallFileName,$width,$heigh){
		...
		if(imagecopyresampled($thumb,$image,0,0,0,0,$width,$heigh,$BigWidth,$BigHeigh)){
			return imagepng($thumb,$smallFileName);
		}
	}
