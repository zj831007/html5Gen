...
class browser extends uploader {
    ...
    public function __construct() {
        ...
        if (isset($this->post['dir'])) {
			...
			$dir = $this->checkInputDir($this->post['dir'], false, false);
        ...
        if (isset($this->get['dir'])) {
			...
			$dir = $this->checkInputDir($this->get['dir'], false, false);
    ...
    protected function act_browser() {
        if (isset($this->get['dir']) &&
			...
			$this->session['dir'] = path::normalize("{$this->get['dir']}");

        return $this->output();
    }

    protected function act_init() {
        $tree = $this->getDirInfo($this->theDir);
		...
		$files = $this->getFiles('');
       // $dirWritable = dir::isWritable("{$this->config['uploadDir']}}");
		$dirWritable = $tree['writable'];
        $data = array(
            'tree' => &$tree,
            'files' => &$files,
    ...
    protected function act_thumb() {
       ...
       // $file = "{$this->thumbsDir}/{$this->type}/{$this->get['dir']}/$file";
		$file = "{$this->thumbsDir}/{$this->get['dir']}/$file";
        if (!is_file($file) || !is_readable($file)) {
            //$file = "{$this->config['uploadDir']}/{$this->type}/{$this->get['dir']}/" . basename($file);
			$file = "{$this->config['uploadDir']}/{$this->get['dir']}/" . basename($file);
            if (!is_file($file) || !is_readable($file))
    ...
    protected function act_renameDir() {
		...
		//$newName = iconv("utf-8","GBK//IGNORE",$newName);
		...
		$thumbDir = "$this->thumbDir/{$this->post['dir']}";
        if (is_dir($thumbDir))
    ...
    protected function act_delete() {
		...
		$thumb = "{$this->thumbsDir}/{$this->post['dir']}/{$this->post['file']}";
           ...
           // if ($type != $this->type) continue;
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                if (is_file($fromThumb) && is_readable($fromThumb)) {
					...
					$toThumb = "{$this->thumbsDir}/{$this->post['dir']}";
                    if (!is_dir($toThumb))
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        /*$prefix = "";
		if (isset($this->get['prefix'])) $prefix = $this->get['prefix'];
        //TODO add file first part by type or mime
		$file_ext = ".".file::getExtension($file['name']);
		$filename = $prefix . date("YmdHis", time()).rand(1000, 9999) .$file_ext;
    ...
    protected function getFiles($dir) {
        ...
        $files = dir::content($dir, array('types' => "file",'followExts' => $this->config['allowTypes'][$this->type]));
    ...
    protected function getDirs($dir) {
        ...
        if (is_array($dirs)) {
            ...
            foreach ($dirs as $cdir) {
				 // == add start
				//$filename = @iconv("UTF-8","GBK",$cdir);
				// == add end
                // $info = $this->getDirInfo($filename);
				$info = $this->getDirInfo($cdir);
				
                if ($info === false) continue;
				//$info['name'] = @iconv("GBK","UTF-8",$info['name']);
