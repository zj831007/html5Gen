...
browser.setTreeData = function(data, path) {
    ...
    if (data.dirs && data.dirs.length) {
        ...
        $.each(data.dirs, function(i, cdir) {
            browser.setTreeData(cdir, path + data.name == "/" ? "" : '/');
        });
...
browser.buildTree = function(root, path) {
    ...
    if (root.dirs) {
        ...
        for (var i = 0; i < root.dirs.length; i++) {
            cdir = root.dirs[i];			
            html += browser.buildTree(cdir, path + root.name == "/" ? "" :'/');
        }
        html += '</div>';
    }
    html += '</div>';
            ...
            $('#loadingDirs').show(200, function() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var html = '';
                        $.each(data.dirs, function(i, cdir) {
                            html += '<div class="folder"><a href="kcdir:/' + 
							        _.escapeDirs(path == "" ? cdir.name : path + '/' + cdir.name) + 
									'"><span class="brace">&nbsp;</span><span class="folder">' + 
									_.htmlData(cdir.name) + '</span></a></div>';
                        });
                        if (html.length) {
